export interface SpeechAnalysis {
  ...
  speechSpeed: number; // words per minute (estimated)
  durationSeconds: number;
}

export class SpeechAgent {
  private fillerList = ['um', 'umm', 'actually', 'basically', 'like', 'you know'];

  /**
   * Analyzes speech statistics from a text transcript and duration.
   */
  public analyzeSpeech(text: string, durationSeconds: number): SpeechAnalysis {
    const words = text.toLowerCase().split(/\s+/);
    const fillerWords: { [key: string]: number } = {};
    let totalFillers = 0;
   ...
   */
  public checkSpeedAdjustmentRequest(text: string): number | null {
